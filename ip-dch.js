...
{ "User-Agent": BASE_UA, "Accept-Language": "en" }
);
...
try {
return await get(url, { "User-Agent": BASE_UA });
} catch (e) {
return "";
}
};
...
if (!country) return "🌐";
if (country.includes("中国") || country === "CN") return "🇨🇳";
if (country.includes("日本") || country === "JP") return "🇯🇵";
if (country.includes("美国") || country === "US") return "🇺🇸";
if (country.includes("香港") || country === "HK") return "🇭🇰";
if (country.includes("台湾") || country === "TW") return "🇹🇼";
if (country.includes("澳门") || country === "MO") return "🇲🇴";
if (country.includes("新加坡") || country === "SG") return "🇸🇬";
if (country.includes("韩国") || country === "KR") return "🇰🇷";
if (country.includes("英国") || country === "GB") return "🇬🇧";
if (country.includes("德国") || country === "DE") return "🇩🇪";
if (country.includes("法国") || country === "FR") return "🇫🇷";
return "📍";
};
...
try {
// 【修改点】海外用户首选：全球分布、IPv6支持完美的 ip.sb 接口
const lRes = await ctx.http.get(
'https://api.ip.sb/geoip',
{ headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 4000, policy: 'DIRECT' }
);
const body = JSON.parse(await lRes.text());
if (body?.ip) {
lIp = body.ip || "获取失败";
const country = getCName(body.country || "");
lLoc = `${getFlagEmoji(country)} ${country} ${body.city || ""}`.trim();
lIsp = fmtISP(body.isp || body.organization);
}
} catch (e) {}

if (lIp === "获取失败") {
try {
// 【修改点】海外用户备用：国际主流的 ipinfo 接口
const resInfo = await ctx.http.get(
'https://ipinfo.io/json',
{ headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 4000, policy: 'DIRECT' }
);
const bodyInfo = JSON.parse(await resInfo.text());
if (bodyInfo?.ip) {
lIp = bodyInfo.ip;
...
if (risk !== null) {
if (risk >= 80) {
riskIPPureTxt = `极高 (${risk})`;
riskIPPureCol = C_RED;
ippSev = 4;
} else if (risk >= 70) {
riskIPPureTxt = `高危 (${risk})`;
riskIPPureCol = C_ORANGE;
ippSev = 3;
} else if (risk >= 40) {
riskIPPureTxt = `中等 (${risk})`;
riskIPPureCol = C_YELLOW;
ippSev = 1;
} else {
riskIPPureTxt = `低危 (${risk})`;
ippSev = 0;
}
}
} catch (e) {}
...
riskIpapiCol =
lv.includes('High') || lv.includes('Very High')
? C_ORANGE
...
const [gptStatus, geminiStatus, youtubeStatus, netflixStatus, tiktokStatus] =
await Promise.all([
checkChatGPT(),
checkGemini(),
checkYouTube(),
...
const iconName = (status === "Cross" || status === "CN")
? "xmark.circle.fill"
...
{
type: 'text',
text: `数据中心 (${policy || 'DCH'})`,
font: { size: HEADER_FONT, weight: 'heavy' },
textColor: C_TITLE,
flex: 1
},
{
type: 'image',
src: `sf-symbol:${summaryIcon}`,
color: summaryCol,
width: 12,
height: 12
},
