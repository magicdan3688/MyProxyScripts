...
export default async function (ctx) {
  ...
  const localArea = localExit.label || "未知地区";
...
function normalizeCarrierName(value) {
  ...
  if (
    raw.includes("中国电信") ||
    /\bchina\s*telecom\b/.test(lower) ||
    /\bchinanet\b/.test(lower)
  ) {
    return "中国电信";
  }
  ...
  if (
    raw.includes("中国广电") ||
    /\bchina\s*broadnet\b/.test(lower) ||
    /\bchina\s*broadcasting\s*network\b/.test(lower)
  ) {
    return "中国广电";
  }
...
function cleanForeignCarrierName(value) {
  const raw = clean(value);
